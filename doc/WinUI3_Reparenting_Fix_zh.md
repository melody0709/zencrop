...
3. **渲染合成彻底崩溃与坐标偏移 Bug (如 Win11 画图、照片应用)：**
...
为了处理旧式嵌套 XAML 应用 (Magpie) 与纯血 WinUI 3 应用 (画图、照片) 之间的冲突，ZenCrop 必须在触碰窗口样式前，精确知道自己面对的是哪种架构。
...
#### 路径 B: 现代纯血 WinUI 3 应用 (画图、照片) -> `m_hasModernXAML == true`

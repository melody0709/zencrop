...
- **大段翻译不再超时**：接收超时改为按推理档位分档（60 s / 120 s），并与 15 s 的连接超时彻底分离。非流式 LLM 要等整段生成结束才发响应头，旧的四参同值等于把 15 s 当成生成上限，大页面必然报 `WinHttpReceiveResponse failed (12002)`。
- **自动重试**：传输类失败自动重试一次（合计最多 2 次尝试），内容/契约类失败另有独立的 1 次额度，偶发波动不必再手动点"重新翻译"；用户取消与鉴权失败不重试。
- **等待有反馈**：翻译过程中状态栏持续累加已等待秒数，重试时文案切换为"请求超时，正在重试…"且秒数不归零。
- **不该翻译的内容本地保留**：整段就是 URL、文件路径、哈希、版本号、代码标识符或纯装饰行时不再发给模型，既省 token，也消除了这类片段最容易触发的"空译文"契约失败。
- **空答案更少**：实测默认采样下该模型会把译文数组尾部返回为空串（34 轮中 6 轮），将 SiliconFlow 的 `DeepSeek-V4-Flash` 默认采样温度设为 0.2 后 32 轮 0 次；并配合 strict JSON schema 与"无可翻译内容时原样返回、绝不返回空串"的提示词。
- **失败可定位**：错误文案恢复显示服务商自己的 trace id（此前被自定义 `X-Trace-Id` 顶掉）；重试/失败会向 `%LOCALAPPDATA%\ZenCrop\translation_diagnostics.log` 追加一行诊断（段数、批数、重试次数、耗时、批次号，不含文本内容）。

...
- **划词翻译手动输入**：未读取到选区时，按 `Shift+A` 会打开原非模态结果窗，可直接在 Preview 所见即所得编辑器中输入或粘贴文字。
- **结构化划词翻译**：模拟复制兜底读取到富文本（HTML）时，代码块、表格与列表结构在翻译前后完整保留，可在结果窗所见即所得编辑（含表格）。
- **紧凑结果窗口**：无边框翻译窗口将语言选择、源码/预览切换与重新识别按钮收进紧凑标题栏；`Ctrl+W` 可直接关闭窗口。
